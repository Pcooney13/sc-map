...
const OldMap = ({
    ...
    // const sortCounties = (countyGeographies) => {
    //     let sortedCounties = countyGeographies.sort((a, b) => {
    //         if (a.id.slice(0, 2) < b.id.slice(0, 2)) {
    //             return -1
    //         }
    //         if (a.id.slice(0, 2) > b.id.slice(0, 2)) {
    //             return 1
...
const SVGCountyLinesGroup = ({
    ...
    //create a group for each each state and create a path inside that group for each county in that state
    return (
        <g
            className={`counties pointer-events-none`}
            // className={`counties transition-all duration-700 ease-in-out ${
            //     thestateCode === '0'
                        ...
                        {singleState.counties.map((county, index) => {
                            return (
                                <motion.path
                                    ...
                                    // className={`${
                                    //     thestateCode !== 0 &&
                                    //     thestateCode === d.id.substring(0, 2)
                                    //         ? 'opacity-100 pointer-events-auto'
                                    //         : 'opacity-0 pointer-events-none'
                                    // } stroke-[0.25] cursor-pointer text-slate-300`}
    ...
    // return (
    //     <g
    //         className={`counties transition-all duration-700 pointer-events-none ease-in-out`}
    //         // className={`counties ${
    //         //     thestateCode === '0'
    //         //         ? 'opacity-0 pointer-events-none'
    //         //         : 'opacity-100 pointer-events-auto'
    //         // }`}
    //     >
    //         {sortCounties(countyGeographies).map((d, i) => (
    //             <motion.path
    //                 whileHover={{
    //                     fill: 'rgba(253, 224, 71, 1)',
    //                 }}
    ...
    //                 title={d.properties.name}
    //                 onClick={(e) => {
    //                     let stateID = d.id.substring(0, 2)
    //                     let state
    //                     stateCodes.forEach((code) => {
    //                         if (code[0] === stateID) {
    //                             state = code[1]
    //                             clickedState(e, d, state)
    //                         }
    //                     })
    //                 }}
    //                 key={`path-${i}`}
